...
import com.kaycheung.order_service.messaging.inbox.InboxEventRepository;
...
@RequestMapping("/api/v1/internal/inbox")
public class InboxController {
    ...
    @PostMapping("/payment-attempt-authorized")
    public ResponseEntity<Void> onPaymentAttemptAuthorized(@RequestBody InboxMessage request) {
        return persistInboxEvent("payment-attempt-authorized", request);
    }

    @PostMapping("/payment-attempt-canceled")
    public ResponseEntity<Void> onPaymentAttemptCanceled(@RequestBody InboxMessage request) {
        return persistInboxEvent("payment-attempt-canceled", request);
    }

    @PostMapping("/payment-attempt-failed")
    public ResponseEntity<Void> onPaymentAttemptFailed(@RequestBody InboxMessage request) {
        return persistInboxEvent("payment-attempt-failed", request);
    }

    @PostMapping("/payment-captured")
    public ResponseEntity<Void> onPaymentCaptured(@RequestBody InboxMessage request) {
        return persistInboxEvent("payment-captured", request);
    }

    @PostMapping("/payment-canceled")
    public ResponseEntity<Void> onPaymentCanceled(@RequestBody InboxMessage request) {
        return persistInboxEvent("payment-canceled", request);
    }
