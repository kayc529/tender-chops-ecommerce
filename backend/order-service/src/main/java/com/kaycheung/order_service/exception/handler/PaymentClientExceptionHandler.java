...
public class PaymentClientExceptionHandler {
    private static final Logger log = LoggerFactory.getLogger(PaymentClientExceptionHandler.class);

    @ExceptionHandler()
    public ResponseEntity<ApiErrorResponse> handlePaymentClientExceptionHandler(PaymentClientException ex, HttpServletRequest request){
        log.error(
